...
function toDC(){
	...
	if [ $? -eq 0 ]; then
		...
		local ip=$(cat .tmp | awk '{print $1}'); local domain=$(cat .tmp | awk '{print $2}')
		unlink /etc/resolv.conf &>/dev/null; echo -e "nameserver ${ip}\nsearch ${domain}" > /etc/resolv.conf 2>/dev/null; echo -e " ${greenColour}listo${endColour}"
	fi

	host -t SRV _kerberos.udp.ej.ies | grep '(NXDOMAIN)' $>/dev/null
