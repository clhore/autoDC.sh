...
function config(){
	...
	local pcIp=$(whiptail --inputbox "Introduce la ip fija para su servidor (Ej: 192.168.1.111/24):" 8 78 --title "autoDC - by Adrián Luján Muñoz" 3>&1 1>&2 2>&3)
...
function firewall(){
	echo -en "${grayColour}:: Abriendo puertos necesarios${endColour}"; sleep 0.5

	local listPorts=(1:65535/tcp 1:65535/udp)

       	for port in "${listPorts[@]}"; do ufw allow $port &>/dev/null; done; echo -e " ${greenColour}listo${endColour}"
}
...
function toDC(){
	cat /.tmp2 &>/dev/null; if [ $? -ne 0 ]; then
		sleep 0.5; firewall; ntpConfig; dependencies; local ip=$(cat .tmp | awk '{print $1}'); local pcDomain=$(cat .tmp | awk '{print $2}')
		...
		sleep 1; mv /etc/samba/smb.conf /etc/samba/smb.conf.save &>/dev/null; samba-tool domain provision --use-rfc2307 --interactive
		sleep 1; cp /var/lib/samba/private/krb5.conf /etc &>/dev/null; clear
		...
		echo -en "${grayColour}:: Creando fichero /etc/resolv.conf${endColour}"; unlink /etc/resolv.conf &>/dev/null
		echo -e "nameserver ${ip}\nsearch ${domain}" > /etc/resolv.conf 2>/dev/null
		...
		echo -en "${grayColour}:: Prueba 1 kerkeros $domain${endColour}"; sleep 0.5

		host -t SRV _kerberos.udp.${domain} | grep '(NXDOMAIN)' &>/dev/null

		if [ $? -eq 0 ]; then
			echo -e " ${redColour}error${endColour}"; echo -en "${grayColour}:: Reparando kerberos${endColour}"; sleep 0.5
			apt install krb5-user -y &>/dev/null; kinit Administrator; klist
			local pid=$(sudo ps -aux | grep samba | grep Ss | xargs | awk '{print $2}'); kill $pid &>/dev/null
			local list=("mask smbd nmbd winbind systemd-resolved" "stop smbd nmbd winbind systemd-resolved" "disable smbd nmbd winbind systemd-resolved" "unmask samba-ad-dc" "start samba-ad-dc" "enable samba-ad-dc")
        		for cmd in "${list[@]}"; do systemctl $cmd $>/dev/null; done; echo -e " ${greenColour}listo${endColour}"
		else
			sleep 0.5; echo -e " ${greenColour}listo${endColour}"
		fi;

		echo -en "${grayColour}:: Instalando krb5-user${endColour}"; sleep 0.5
		apt install krb5-user -y &>/dev/null

		if [ $? -ne 0 ]; then
			echo -e " ${redColour}error${endColour}"; echo -en "${grayColour}:: Matado procesos en conflicto${endColour}"; sleep 0.5
                	systemctl stop systemd-resolved &>/dev/null; systemctl disable systemd-resolved &>/dev/null
		...
		echo -en "${grayColour}:: Instalando krb5-user${endColour}"; sleep 0.5
        	apt install krb5-user -y &>/dev/null; sleep 0.5; echo -e " ${greenColour}listo${endColour}" &>/dev/null
	fi

	local ip=$(cat .tmp | awk '{print $1}'); local pcDomain=$(cat .tmp | awk '{print $2}'); local d1=$(echo "${pcDomain}" | awk -F '.' '{print $2}')
	local d2=$(echo "${pcDomain}" | awk -F '.' '{print $3}'); local domain=$(echo "${d1}.${d2}")
	...
	rm .tmp /.tmp2 2>/dev/null; checkSamba
	echo -e "${grayColour}:: El equipo se${endColour} ${redColour}reiniciara en 4s${endColour}"; sleep 4; reboot
