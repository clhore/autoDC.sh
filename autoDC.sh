...
function toDC(){
	...
	echo -e "domain ${domain}\nsearch ${ip}" > /etc/resolv.conf 2>/dev/null
	...
	if [ $? -eq 0 ]; then
		...
		rm /etc/resolv.conf &>/dev/null; echo -e "domain ${domain}\nsearch ${ip}" > /etc/resolv.conf 2>/dev/null; echo -e " ${greenColour}listo${endColour}"
	fi

	echo -en "${grayColour}:: Prueba kerkeros $domain${endColour}"; sleep 0.5
	...
	fi; rm .tmp 2>/dev/null; checkSamba
