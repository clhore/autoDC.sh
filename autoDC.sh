...
function toDC(){
	...
	echo -e "domain ${domain}\n${ip}\nsearch 127.0.0.1" > /etc/resolv.conf 2>/dev/null
	...
	if [ $? -eq 0 ]; then
		...
		systemctl stop systemd-resolved &>/dev/null; systemctl disable systemd-resolved &>/dev/null; rm /etc/resolv.conf &>/dev/null
		echo -e "domain ${domain}\n${ip}\nsearch 127.0.0.1" > /etc/resolv.conf 2>/dev/null; echo -e " ${greenColour}listo${endColour}"
	fi
